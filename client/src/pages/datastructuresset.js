...
function Dpproblemset() {
  ...
  useEffect(() => {
    if (!data_structures) getdsProblems();
  }, []);

  const rendersProblem = (dsproblem) => {
    const genLink = `https://codeforces.com/problemset/problem/${dsproblem.contestId}/${dsproblem.index}`;
    return (
      <tr>
        <td>{dsproblem.name}</td>
        <td>{dsproblem.index}</td>
        <td>{dsproblem.contestId}</td>
        <a href={genLink}>
          <td>Solve</td>
        </a>
      </tr>
                  ...
                  <table class="table table-hover text-nowrap table-striped ">
                    ...
                    <tbody>
                      {data_structures && data_structures.length > 0 ? (
                        rendersProblem(data_structures[0])
                      ) : (
